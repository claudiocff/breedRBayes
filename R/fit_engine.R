...
#'   functions: `mrk(f, M, method)` genomic effect from a **marker matrix**
...
#'   genotypes, else RR-BLUP — the two are prediction-equivalent). In GBLUP mode
#'   the genomic relationship is fitted through a low-rank principal-component
#'   rotation keeping the leading components that explain `exp_var_rank` of the
#'   genomic variance (see [bbglr_control()], default 0.99); when \pkg{RSpectra}
#'   is installed this rotation is computed straight from the markers without ever
#'   forming the `n x n` relationship matrix (much cheaper for many genotypes). A
#'   `mrk()` fit
